...
def acreate_profile(request):
    if request.method == 'POST':
        ...
        Profile.objects.filter(id = user_id).create(user_id=user_id,phonenumber=phonenumber, first_name=first_name, last_name=last_name, bio=bio, birth_date=birth_date, avatar=avatar, hobby=hobby, city=city, country=country)
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'dashboard/instructor/edit_profile.html', context)
...
def publish_tutorial(request):
    if request.method == 'POST':
        ...
        except (KeyError, MultiValueDictKeyError, Http404) as e:
            messages.error(request, f'Error publishing tutorial: {e}')
...
def publish_notes(request):
    if request.method == 'POST':
        ...
        a.save()
        messages.add_message(request, messages.SUCCESS, 'Notes Was Published Successfully')
        return redirect('lnotes')
    else:
        messages.add_message(request, messages.ERROR, 'Notes Was Not Published Successfully')
